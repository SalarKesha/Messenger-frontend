...
export default function GroupPopup({ handlePopup }) {
    ...
    const createNewGroupChat = () => {
        if (imageRef.current.files[0] && nameInput) {
            ...
            console.log('mode 1');
            postReq('group/create/', formData).then(({ data, status }) => {
            ...
            console.log('mode 1');
            postReq('group/create/', { name: nameInput }).then(({ data, status }) => {
                notify('Group created successfully', 'success')
                setRoom(state => {
                    return {
                        id: data.id,
            ...
            notify('Please fill the group name')
        }
    }
    return (
        <div className='group-popup-overlay  popup-overlay' onClick={() => handlePopup(false)}>
            <div className='popup-container' onClick={handleContainerClick}>
                <div className="input-container">
                    <input placeholder='Enter Group Name' type="text" className="group-name" value={nameInput} onChange={handleNameInput} />
                ...
                <div className="button-container">
                    ...
                    <button className='accept' onClick={createNewGroupChat}>
