...
export default function ExploreGroupPopup({ group, name, handleJoin, handlePopup }) {
    const { postReq } = useRequest()
    const handleContainerClick = (e) => {
        e.stopPropagation()
    }
    const createNewGroupMember = () => {
        postReq('group/member/create/', { group: group }).then(({ data, status }) => {
            handleJoin(data.group)
        }).catch(() => {
    ...
    return (
        <div className='explore-popup-overlay popup-overlay' onClick={() => handlePopup(false)}>
            <div className='popup-container' onClick={handleContainerClick}>
                <p className="content">Do you want join to '{name}' group ?</p>
                <div className="button-container">
                    ...
                    <button className='accept' onClick={createNewGroupMember}>
                        <span>Continue</span>
