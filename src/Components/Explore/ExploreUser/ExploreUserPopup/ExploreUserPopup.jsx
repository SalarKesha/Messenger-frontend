...
export default function ExploreUserPopup({ user, name, handleRoom, handlePopup }) {
    const { postReq } = useRequest()
    const handleContainerClick = (e) => {
        e.stopPropagation()
    }
    const createNewPrivateChat = () => {
        postReq('private/create/', { user: user }).then(({ data, status }) => {
            handleRoom(data.id)
        }).catch(() => {
    ...
    return (
        <div className='explore-popup-overlay popup-overlay' onClick={() => handlePopup(false)}>
            <div className='popup-container' onClick={handleContainerClick}>
                <p className="content">Do you want start chat with {name} ?</p>
                <div className="button-container">
                    ...
                    <button className='accept' onClick={createNewPrivateChat}>
                        <span>Continue</span>
